/**
 * Component 5.1 eval spine — posts evaluation queries to deployed Worker webhook.
 *
...
async function main(): Promise<void> {
  if (!hasConfig) {
    console.warn(
      "[eval] Skipped — set WORKER_WEBHOOK_URL and WEBHOOK_SECRET in .dev.vars",
    );
    process.exit(0);
  }

  const csvPath = resolve(ROOT, "evaluationqueries.csv");
  const rows = parseQueriesCsv(csvPath);
  const baseUpdateId = Date.now();

  console.log(`[eval] Posting ${rows.length} queries to webhook`);
  console.log(`[eval] Wait per query: ${EVAL_WAIT_MS}ms (set EVAL_WAIT_MS to override)`);
  console.log("---");
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    console.log(`[eval] Posted ${row.id} update_id=${updateId} query="${row.query}"`);
  ...
  console.log("---");
  console.log("[eval] Done. Export traces and run sql/agent-trace.sql for each update_id:");
