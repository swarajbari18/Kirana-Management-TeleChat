...
describe("verifyBindings BV catalog", () => {
  ...
  it("BV-10 outcomeBinding valid denied", () => {
    ...
    expect(verifyBindings(response, mspRegistry(), outcomes)).toHaveLength(0);
  });
  ...
  it("BV-11 outcomeBinding unknown id", () => {
    ...
    expect(
      verifyBindings(response, mspRegistry(), emptyOutcomes)[0]?.reason,
